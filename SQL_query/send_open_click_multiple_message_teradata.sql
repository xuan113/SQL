-- create dummy table
CREATE TABLE sbox_play.alexl_dummy_6555 AS(
SELECT DISTINCT message_id
FROM gdw_prod.msg_send_f
WHERE message_id IN (917, 904, 906, 911, 915, 918, 920, 923, 925, 901, 949, 953, 956, 958, 10189)
AND send_utc_date >= '2015-06-30') WITH DATA

  -- Execute the following query in Teradata
DROP TABLE sbox_play.alexl_soc_multi_6555;

CREATE TABLE sbox_play.alexl_soc_multi_6555 AS
(SELECT 
   a.test_id,
   a.test_cell_nbr,
   a.account_id,
...
FROM
  (SELECT test_id, test_cell_nbr, account_id, message_id
  FROM(
     SELECT test_id, account_id,test_cell_nbr 
     FROM gdw_prod.ab_exp_cust_allocs_f
     WHERE test_id = 6555
     AND allocation_utc_date >= '2015-06-30'
     AND deallocation_date IS NULL) alloc
   CROSS JOIN sbox_play.alexl_dummy_6555 dummy
   WHERE dummy.message_id IN (917, 904, 906)) a      
LEFT OUTER JOIN
...
USE alexl;
DROP TABLE IF EXISTS alexl.msg_multi_${testid};
CREATE EXTERNAL TABLE alexl.msg_multi_${testid}
  (test_id BIGINT,
   test_cell_nbr INT,
   account_id BIGINT,
   message_id INT,
   ...
   click_cnt INT)
STORED AS TEXTFILE
LOCATION "s3n://netflix-dataoven-prod-users/alexl/experiment_6741/msg_multi_6741";

 
USE alexl;
DROP TABLE IF EXISTS alexl.msg_${testid}_multi_shiny;
CREATE TABLE IF NOT EXISTS alexl.msg_${testid}_multi_shiny AS
SELECT 
  a.test_id,
  a.test_name,
  a.test_cell_nbr,
